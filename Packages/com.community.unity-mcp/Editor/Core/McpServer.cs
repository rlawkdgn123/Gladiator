...
namespace Community.Unity.MCP
{
    ...
    public class McpServer
    {
        ...
        static McpServer()
        {
            ...
            // Gracefully stop the server before domain reload to avoid ThreadAbortException errors
        ...
        private void ListenLoop()
        {
            while (_isRunning)
            {
                ...
                {
                    // Domain reload — exit silently, not an error
                    break;
                }
                catch (Exception ex)
                {
        ...
        private void HandleMessage(HttpListenerContext context)
        {
            ...
            // Empty responseBody means it was a notification — send 200 with no body so bridge.js
            // receives nothing and doesn't forward anything to Claude Desktop (avoids ZodErrors).

    ...
    public static class JsonRpcHandler
    {
        ...
        public static string ProcessRequest(string requestJson)
        {
            try
            {
                ...
                // Notifications don't require a response - returning empty string prevents null-id ZodErrors
                if (!string.IsNullOrEmpty(method) && method.StartsWith("notifications/"))
                    return string.Empty;
                ...
                switch (method)
                {
                    case "initialize":
                        ...
                        break;
                    case "tools/list":
                        // Build manually to avoid JsonUtility double-encoding properties as string
                        return CreateRawSuccessResponse(id, BuildToolsListJson());
                    case "tools/call":
        ...
        /// <summary>
        /// Create a success response where the result is already a pre-built JSON string.
        /// Used for responses that need raw JSON (e.g. tools/list with proper object schema).
        /// </summary>
        private static string CreateRawSuccessResponse(string id, string resultJson)
        {
            var sb = new StringBuilder();
            sb.Append("{\"jsonrpc\":\"2.0\",");
            AppendId(sb, id);
        ...
        /// <summary>
        /// Builds the tools/list result JSON manually so that inputSchema.properties is
        /// emitted as a JSON object rather than an escaped string (JsonUtility limitation).
        /// </summary>
        private static string BuildToolsListJson()
        {
            ...
            for (int i = 0; i < tools.Length; i++)
            {
                ...
                var t = tools[i];
                sb.Append("{");
                sb.Append($"\"name\":\"{EscapeJson(t.name)}\",");
                sb.Append($"\"description\":\"{EscapeJson(t.description)}\",");
                sb.Append("\"inputSchema\":{\"type\":\"object\",\"properties\":");
                sb.Append(t.inputSchema.properties ?? "{}");
                if (t.inputSchema.required != null && t.inputSchema.required.Length > 0)
                {
                    sb.Append(",\"required\":[");
                    for (int j = 0; j < t.inputSchema.required.Length; j++)
                    {
                        if (j > 0) sb.Append(",");
                        sb.Append($"\"{EscapeJson(t.inputSchema.required[j])}\"");
                    }
                    sb.Append("]");
                }
                sb.Append("}}");
        ...
        /// <summary>
        /// Appends the id field as a number when possible (Claude Desktop Zod requires number, not string).
        /// </summary>
        private static void AppendId(StringBuilder sb, string id)
        {
            if (!string.IsNullOrEmpty(id))
            {
                if (long.TryParse(id, out long numericId))
                    sb.Append($"\"id\":{numericId},");   // Numeric — Claude Desktop Zod requires this
                else
                    sb.Append($"\"id\":\"{id}\",");      // Non-numeric string id (rare)
            }
            else
            {
                sb.Append("\"id\":null,");

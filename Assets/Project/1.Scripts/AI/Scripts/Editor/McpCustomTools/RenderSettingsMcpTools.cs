...
namespace Community.Unity.MCP
{
    ...
    public static class RenderSettingsMcpTools
    {
        ...
        public class SetPostProcessingArgs
        {
            ...
            [McpParam("Property name on the effect. bloom: intensity/threshold/scatter/clamp | color_adjustments: postExposure/contrast/saturation/hueShift/tint | vignette: intensity/smoothness/rounded/color | depth_of_field: focusDistance/aperture/focalLength/mode | chromatic_aberration: intensity | motion_blur: intensity/clampValue(→clamp)/mode/quality | film_grain: intensity/response | tonemapping: mode | white_balance: temperature/tint", Required = true)]
            ...
            switch (args.effect.ToLower())
            {
                ...
                case "motion_blur":
                {
                    ...
                    switch (args.property.ToLower())
                    {
                        ...
                        case "clampvalue":  mb.clamp.overrideState     = setOverride; mb.clamp.value     = args.floatValue; break;
                        case "mode":

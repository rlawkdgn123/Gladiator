...
/// 
///  - 마우스 위치에 따른 3분할.
///  - Update()를 쓰므로 MonoBehaviour 싱글톤임.
///  - 그러므로 씬 객체로 존재해야 함.
//////////////////////////////////////////////////
using UnityEngine;
...
public class CursorManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // 마우스 공급
        if (m_inputHandler != null)
            currentLookVector = m_inputHandler.GetLookVector();

        // 마우스 3분할
        if (Mouse.current != null)
        {
            currentMouseScreenPos = Mouse.current.position.ReadValue();
            GuardZoneDirection = GuardZoneCalculator(currentMouseScreenPos, m_deadZoneRadius);
        }
    }

    private GuardZone GuardZoneCalculator(Vector2 mouseScreenPos, float radius)
    {
        // 화면 중앙을 기준으로 마우스 방향을 구하기.
        // 즉 "현재 마우스가 화면 중앙에서 어느 방향을 보고 있느냐"를 판단하기 위한 준비 단계임.
        Vector2 center = new Vector2(Screen.width * 0.5f, Screen.height * 0.5f);
        Vector2 dir = mouseScreenPos - center;

        // 중앙 근처의 작은 흔들림은 섹터를 확정하지 않음 주의.
        // dead zone 안에서는 방향이 계속 바뀌는 현상을 막기 위해 None을 반환.
        if (dir.sqrMagnitude <= radius * radius)
            return GuardZone.None;

        // 내적 비교는 "방향"만 중요하므로 길이를 1로 맞춰줌.
        dir.Normalize();

        // Top / Left / Right 세 기준축과 현재 마우스 방향의 내적.
        // 내적값이 클수록 두 벡터가 더 비슷한 방향을 보고 있다는 뜻.
        float topDot = Vector2.Dot(dir, m_topDir);
        float leftDot = Vector2.Dot(dir, m_leftDir);
        float rightDot = Vector2.Dot(dir, m_rightDir);

        // 세 축 중 가장 가까운 축을 현재 마우스 섹터로 판정.
    ...
    public GuardZone GetGuardZoneDirection()
    {
        return GuardZoneDirection;
    }
